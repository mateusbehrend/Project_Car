...
void loop()
{
 ...
 /*
  ...
  if (average() > 100 && average() < 1700){
    ...
    kd =0.0475;
    if (startingPoint ==1){
      ChangeWheelSpeeds(currentLeftSpeed, 90, currentRightSpeed, 90);
      startingPoint++;
    }
    currentLeftSpeed = 90;
  ...
  */
  //Straight Away
  if (average() >= 1800 && average() < 3300){
    //digitalWrite(LED_RF,HIGH);
    if (straightAwayCounter == 0){
        ChangeWheelSpeeds(currentLeftSpeed, 255 , currentRightSpeed, 255);
        straightAwayCounter++;
    }
  ...
  //First Break Discontinuity
   if (average() >= 3400 && average() <= 3900){
    ...
    if (firstBreakCounter == 0){
      ChangeWheelSpeeds(currentLeftSpeed, 60, currentRightSpeed, 60);
      firstBreakCounter++;
    }
    currentLeftSpeed =60;
 ...
 if (average() >= 3600 && average() <= 4000){
  ...
  if (average() >= 3900 && average() < 5900 ){
    //digitalWrite(LED_RF,LOW);
    if (firstBreakCounter == 1){
      ChangeWheelSpeeds(currentLeftSpeed, 180, currentRightSpeed, 180);
      firstBreakCounter++;
    }
  ...
  //Second Disc
  if (average() >= 6000 && average() <= 6400){
    digitalWrite(LED_RF,LOW);
    if (firstBreakCounter == 2){
      weights[0] =-15;
      weights[7] = 15;
      ChangeWheelSpeeds(currentLeftSpeed, 100, currentRightSpeed, 100);
      firstBreakCounter++;
    }
    currentLeftSpeed =100;
    currentRightSpeed =100;
