...
void setup()
{
  ...
  //rightWheelSpeed = 50;
  currentLeftSpeed = 50;
  currentRightSpeed = 50;
  ChangeWheelSpeeds(0, currentLeftSpeed, 0, currentRightSpeed);
  prevTotalSum = 0;
  totalSum =0;
  blackSum = 0;
  prevBlackSum =0;
...
void loop()
{
  ...
  if (average() < 150){
    if (startingPoint ==0){
      ChangeWheelSpeeds(currentLeftSpeed, 25, currentRightSpeed, 25);
      currentLeftSpeed = 25;
  ...
  if (average() > 150 && average()< 300){
    kp = 0.0025;
    kd =0.0008;
    if (startingPoint ==1){
      ChangeWheelSpeeds(currentLeftSpeed, 50, currentRightSpeed, 50);
      currentLeftSpeed = 50;
      currentRightSpeed =50;
      startingPoint++;
    }
  ...
  if (average() >= 4000 && average() < 5800 && firstBreakCounter == 1){
    digitalWrite(LED_RF,LOW);
    ChangeWheelSpeeds(currentLeftSpeed, 50, currentRightSpeed, 50);
    currentLeftSpeed =50;
    currentRightSpeed =50;
  ...
  if (average() >= 5800 && average() <= 6200){
   ...
   if (average() >= 6200 && firstBreakCounter == 3){
    digitalWrite(LED_RF,LOW);
    ChangeWheelSpeeds(currentLeftSpeed, 50, currentRightSpeed, 50);
    currentLeftSpeed =50;
    currentRightSpeed =50;
  ...
  //Serial.print(prevBlackSum);
  //Serial.println();
  //Serial.print(blackSum);
  //Serial.println();

  //END
  if (prevBlackSum >= 2500*6 && blackSum >= 2500*6 && turnAroundCounter == 1){ //only using 6 sensors
  ...
  if (prevBlackSum >= 2500*6 && blackSum >= 2500*6 && turnAroundCounter == 0){ //only using 6 sensors
    ChangeWheelSpeeds(currentLeftSpeed, 0, currentRightSpeed, 0);
    digitalWrite(left_nslp_pin, LOW);
    digitalWrite(right_nslp_pin,LOW);
    digitalWrite(LED_RF,HIGH);
